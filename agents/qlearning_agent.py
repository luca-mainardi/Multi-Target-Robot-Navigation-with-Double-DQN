...
class QLearningAgent(BaseAgent):
    ...
    def train(self, iters):
        ...
        state, tables_to_visit = self.env.reset()
        self.inject_episode_table_list(tables_to_visit)
        ...
        for iteration in trange(iters):
            ...
            for i in range(max_step):
                ...
                next_state, reward, info, table_or_kitchen_number = self.env.step(action, self.current_visit_list)

                # Update expected Q matrix and move to the next state
                self.update(state, next_state, reward, info['actual_action'], table_or_kitchen_number)
                state = next_state
                cum_reward += reward

                #If we reach the max amount of steps or we have visited every table we end the episode
                if i == max_step - 1 or self.visited_all_tables():
                    self.env.reset()
                    break
