...
def parse_args():
    ...
    p.add_argument("GRID", type=Path, help="Paths to the grid file to use. There can be more than " "one.")
    p.add_argument("--no_gui", action="store_true", help="Disables rendering to train faster")
    p.add_argument("--sigma", type=float, default=0.0, help="Sigma value for the stochasticity of the environment.")
    p.add_argument("--fps", type=int, default=30, help="Frames per second to render at. Only used if " "no_gui is not set.")
    p.add_argument("--train_iter", type=int, default=1000, help="Number of training iterations to go through.")
    p.add_argument("--eval_iter", type=int, default=10, help="Number of evaluation iterations to go through.")
    p.add_argument("--random_seed", type=int, default=0, help="Random seed value for the environment.")
    p.add_argument("--agent_type", type=str.lower, default="ddqn", help="Type of agent to use.")
    p.add_argument("--load_model_path", type=Path, default=None, help="Path to a pre-trained model to load.")
    p.add_argument("--n_plates", type=int, default=10, help="Total number of plates to deliver per episode.")
    p.add_argument("--trainable", type=bool, default=True, help="If the agent should continue training or not.")
    p.add_argument("--capacity", type=int, default=3, help="Number of plates an agent can carry at a time.")
    p.add_argument("--experiment_name", type=str, default=None, help="Optional experiment name.")
    p.add_argument(
    ...
    p.add_argument("--start_epsilon", type=float, default=0.8, help="Initial value or epsilon.")
    p.add_argument(
...
def run_episode(env, agent, max_steps):
    ...
    for step in range(max_steps):
        ...
        if isinstance(agent, DoubleDQNAgent):
            q_values.append(torch.max(agent.policy_net(torch.tensor(encode_input(env, state, agent.current_visit_list), device=agent.device, dtype=torch.float32))).item())
...
def main(
    ...
    # Log warnings/errors based on command line arguments given
    ...
    env = Environment(grid, no_gui, sigma=sigma, target_fps=fps, random_seed=random_seed, n_plates=n_plates, agent_start_pos=None)
        ...
        # Train/evaluate the agent
            ...
            if agent.should_train:
                ...
                for ep in range(train_iter):
                    ...
                    # Early stopping
                    best_avg_reward, patience_counter = early_stopping(training_metrics["Total reward"], best_avg_reward, patience_counter)
